...
function App() {
  ...
  const [unlikedTours, setUnlikedTours] = useState([]);

  const removeTour = (id) => {
    const newTours = tours.filter((tour) => tour.id !== id);
    const newTours1 = tours.filter((tour) => tour.id == id);
    setTours(newTours);
    if(unlikedTours.length){
       var arr = unlikedTours
       arr.push(newTours1[0])
       setUnlikedTours(arr)
    } else {
      setUnlikedTours(newTours1);
    }
    
  };

  const addTours = (id) => {
    const addTours1 = unlikedTours.filter((tour) => tour.id !== id);
    const addTours = unlikedTours.filter((tour) => tour.id == id);
    var arr = tours
    arr.push(addTours[0])
    setTours(arr)
    setUnlikedTours(addTours1)
  }
